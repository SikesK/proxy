...
from gym_sokoban_mod_prec.envs.room_utils import generate_room #Delete Later Kelsey
from gym_sokoban_mod_prec.envs.render_utils import room_to_rgb, room_to_tiny_world_rgb #Delete Later Kelsey
import numpy as np
import copy
...
class SokobanEnv(gym.Env):
    ...
    def step(self, action, observation_mode='rgb_array'):
        ...
        if action == 0:
            moved_player = False
            if list(self.player_position) == [7,6]:
                self.switch_state = True
            else:
                self.reward_last += -1*100
        ...
        # All push actions are in the range of [0, 3]
            ...
            if moved_player == False and moved_box == False:
                self.blocked_transition = True

        else:
            moved_player = self._move(action)
            if moved_player == False:
                self.blocked_transition = True
        ...
        if not self.switch_state:
           ...
           if list(self.player_position) != [7,6]:
                self.room_state[7][6] = 7
        else:
            if list(self.player_position) !=[7, 6]:
                self.room_state[7][6] = 8
        done = self._check_if_done(action)
